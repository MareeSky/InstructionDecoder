...
int main(){ // Reads file and runs two decoder functions
      ...
      int j = 0;
	    char hex = 0;
	    fpReadFile = fopen("mem_in.txt", "r"); // Opens file

	    while ((hex = getc(fpReadFile)) != EOF){
            if (hex > 47 && hex < 58){
                mem[i] = (hex - 48) * 16;
                hex = getc(fpReadFile);
	    ...
	    if( fpReadFile == NULL ) //error checking
        {
	    ...
	    while(PC < sizeof(mem) && (mem[PC]) != 0x19){
